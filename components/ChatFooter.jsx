...
const ChatFooter = () => {
  ...
  const { data, isTyping, editMsg, seteditMsg, inputText, setInputText, setAttachment, attachmentPreview, setAttachmentPreview } = chatProviderContext()
  ...
  return (
    <div className='flex items-center bg-c1/[0.5] p-2 rounded-xl relative'>
      {attachmentPreview && (<div className="absolute w-[100px] h-[100px] bottom-16 bg-c1 p-2 rounded-md ">
        <img src={attachmentPreview} alt="image" />
      ...
      <div className='shrink-0'>
        ...
        <label htmlFor='fileUploader' >
          <Icon
            size="large"
            icon={<CgAttachment title='Attachment' size={20} className='text-c3' />}
          />
        </label>
      </div>
      ...
      {isTyping && (
        <div className="absolute -top-6 bg-c2 w-full h-6">
      ...
      {editMsg && (<div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-c4 flex items-center gap-2 py-2 px-4 pr-2 rounded-full text-sm font-semibold cursor-pointer shadow-lg" onClick={() => seteditMsg(false)}>
