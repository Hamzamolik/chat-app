...
const Avatar = ({ size, user, onClick }) => {
    ...
    return (
        <div className={`${c} rounded-full flex items-center justify-center text-base flex-shrink-0 relative`}
            style={{ backgroundColor: user.color }}
            onClick={onClick}
        >
            {user?.isOnline && (<>
                {size === "large" && (<span className='w-[10px] h-[10px] bg-green-500 absolute right-[2px] bottom-[2px] rounded-full' />)}

                {size === "x-large" && (<span className='w-[10px] h-[10px] bg-green-500 absolute right-[3px] bottom-[3px] rounded-full' />)}
            </>)}
            {user?.photoURL ? (
